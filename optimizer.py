...
class SGDMomentum(object):
    ...
    def update_lr(self, itr):
        self.alpha=slef.epsilon/((1+self.gamma*itr)**self.power)
        return self.alpha

    def step(self):
        for group in self.parameters:
            for param in self.parameters[group]:
                self.theta=(self.mu*self.theta) + self.lr*(self.parameters[group][param].grad + self.weight_decay)
                self.parameters[group][param].value -= self.theta
